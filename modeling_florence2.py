...
class PromptGenBlock(nn.Module):
    def __init__(self, prompt_dim=128, prompt_len=8, prompt_seq_len=96, lin_dim=192):
        super(PromptGenBlock, self).__init__()
        # Change spatial dimension to sequence dimension
        self.prompt_param = nn.Parameter(torch.rand(1, prompt_len, prompt_dim, prompt_seq_len))
        self.linear_layer = nn.Linear(lin_dim, prompt_len)
        # 1D convolution operation
        self.conv3x3 = nn.Conv1d(prompt_dim, prompt_dim, kernel_size=3, stride=1, padding=1, bias=False)
        
    def forward(self, emb):
        # emb's shape is [B, C], which is already global representation
        B, C = emb.shape
        
        # Calculate prompt weights
        prompt_weights = F.softmax(self.linear_layer(emb), dim=1)  # Shape is [B, prompt_len]
        
        # Expand dimensions and apply weights
        prompt = prompt_weights.unsqueeze(-1).unsqueeze(-1) * self.prompt_param.repeat(B, 1, 1, 1)
        
        # Sum over prompt template dimension
        prompt = torch.sum(prompt, dim=1)  # Shape is [B, prompt_dim, prompt_seq_len]
        
        # Apply 1D convolution
        prompt = self.conv3x3(prompt)  # Shape is still [B, prompt_dim, prompt_seq_len]
        
        # Average over sequence dimension to get global representation
        prompt_global = prompt.mean(dim=2)  # Shape is [B, prompt_dim]
        
        return prompt_global  # Return global representation, shape is [B, prompt_dim]
class DaViTFiLMLayer(nn.Module):
    def __init__(self, vision_dim, llm_dim):
        super().__init__()
        # Projection layer for generating modulation parameters
        self.scale = nn.Linear(llm_dim, vision_dim)
        self.shift = nn.Linear(llm_dim, vision_dim)
        self.prompt_gen = PromptGenBlock(prompt_dim=llm_dim, prompt_len=8, prompt_seq_len=64, lin_dim=llm_dim)

        # self._init_weights(self.scale)
        # self._init_weights(self.shift)
    ...
    def forward(self, x, input_size, average_language_embedding):
        # Calculate modulation parameters
        prompt_global = self.prompt_gen(average_language_embedding)
        gamma = self.scale(prompt_global)  # (batch_size, vision_dim)
        beta = self.shift(prompt_global)   # (batch_size, vision_dim)
        ...
        # print(f"Scale: W mean={self.scale.weight.mean().item():.4f} W std={self.scale.weight.std().item():.4f} B mean={self.scale.bias.mean().item():.4f}, Shift: W mean={self.shift.weight.mean().item():.4f} W std={self.shift.weight.std().item():.4f} B mean={self.shift.bias.mean().item():.4f}")
        return x, input_size

class MySequential(nn.Sequential):
    def forward(self, x, size, average_language_embedding=None):
        for module in self:
            if isinstance(module, DaViTFiLMLayer):
                x, size = module(x, size, average_language_embedding)
            elif isinstance(module, MySequential):
                x, size = module(x, size, average_language_embedding)  # Pass language_embedding
            else:
...
class DaViT(nn.Module):
            ...
            block = MySequential(
                *[
                    MySequential(OrderedDict([
                        ...
                        (
                            'film_layer', DaViTFiLMLayer(
                                vision_dim=embed_dims[i],
                                llm_dim=llm_dim
                            )
                        ),
                        (
    ...
    #     return x
    def forward_features_unpool(self, x, average_language_embedding=None):
        """
        forward until avg pooling 
        Args:
            x: input image tensor
            language_embedding: language model embeddings for FiLM
        """
        input_size = (x.size(2), x.size(3))
        for conv, block in zip(self.convs, self.blocks):
            x, input_size = conv(x, input_size)
            if self.enable_checkpoint:
                # 注意：如果使用checkpoint，需要修改checkpoint函数以支持额外参数
                x, input_size = checkpoint.checkpoint(block, x, input_size, average_language_embedding)
            else:
                # 传递average_language_embedding参数给block
                x, input_size = block(x, input_size, average_language_embedding)
        return x
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    ...
    def _encode_image(self, pixel_values, inputs_embeds):
        if len(pixel_values.shape) == 4:
            batch_size, C, H, W = pixel_values.shape
            T = 1
            x = self.vision_tower.forward_features_unpool(pixel_values, inputs_embeds)
        else:
            raise ValueError(f'invalid image shape {pixel_values.shape}')
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                average_language_embedding = inputs_embeds.mean(dim=1)
                image_features = self._encode_image(pixel_values, average_language_embedding)
